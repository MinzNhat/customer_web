...
import DetailOrder from "./DetailStaff";
...
const CheckTable = (props: Props) => {
    ...
    const handleDelete = async () => {
        ...
        selectedRows.forEach((rows) => {
            rows.forEach(rowIndex => {
                selectedIds.push(tableData[rowIndex].staffId);
            });
        });
        let error = false;
        ...
        const responses = await Promise.all(selectedIds.map(async (id) => {
            if (id) {
                const response = await staffOperation.deleteStaff({ staffId: id });
        ...
        if (error) {
            setMessage("Đã có lỗi xảy ra khi xoá nhân viên.");
            setOpenError(true);
        } else {
    ...
    return (
        <Card className={"h-full w-full overflow-auto no-scrollbar p-4"}>
            {openAdd && <AddStaff setOpenAdd={setOpenAdd} reloadData={reloadData} />}
            ...
            outline-none dark:bg-[#3A3B3C]  pl-1 pr-3`}
              ...
              linear justify-center rounded-lg font-medium dark:font-base transition duration-200`}
                            ...
                            <MdOutlineAddCircleOutline /><p className="flex gap-1">Thêm <p className="hidden sm:block">nhân viên</p></p>
                        </Button>
              ...
              linear justify-center rounded-lg font-medium dark:font-base transition duration-200`}
                                onClick={() => { setMessage(`Xác nhận xoá ${Array.from(selectedRows.values()).reduce((total, set) => total + set.size, 0)} nhân viên đã chọn?`); setOpenDelete(true) }}>
                                    ...
                                    <tr {...headerGroup.getHeaderGroupProps()} key={index}>
                                        {headerGroup.headers.map((column, index) => (
                                            <th
                                                ...
                                                key={index}
                                            >
                                                <div className={`text-xs font-bold tracking-wide text-gray-600 lg:text-xs whitespace-nowrap ${column.render("Header") == "detail" ? "text-end" : (column.render("Header") == "active" ? "text-center pr-2" : "text-start pr-2")}`}>
                                    ...
                                    >
                                        {row.cells.map((cell, cellIndex) => {
                                            ...
                                            if (cell.column.Header === "Checkbox") {
                                                ...
                                                renderData = (
                                                    <p className="pr-2 text-sm font-bold dark:text-white whitespace-nowrap flex justify-center">
