...
import { AgencyOperation, VehicleOperation } from "@/TDLib/main";
import AddPostOffice from "./AddPostOffices";
...
const CheckTable = (props: Props) => {
    ...
    const handleDelete = async () => {
        ...
        selectedRows.forEach((rows) => {
            rows.forEach(rowIndex => {
                selectedIds.push(tableData[rowIndex].agencyId);
            });
        });
        let error = false;
        ...
        const responses = await Promise.all(selectedIds.map(async (id) => {
            if (id) {
                const response = await agencyOperation.deleteAgency(id)
        ...
        if (error) {
            setMessage("Đã có lỗi xảy ra khi xoá bưu cục.");
            setOpenError(true);
        } else {
            setMessage("Xoá các bưu cục - đại lý đã chọn thành công.");
            setOpenReload(true);
    ...
    return (
        <Card className={"h-full w-full p-4 overflow-auto no-scrollbar"}>
            {openAdd && <AddPostOffice reloadData={reloadData} setOpenAdd={setOpenAdd} />}
            ...
            outline-none dark:bg-[#3A3B3C]  pl-1 pr-3`}
              ...
              linear justify-center rounded-lg font-medium dark:font-base transition duration-200`}
                            ...
                            <MdOutlineAddCircleOutline /><p className="flex gap-1">Thêm <p className="hidden sm:block">bưu cục</p></p>
                        </Button>
              ...
              linear justify-center rounded-lg font-medium dark:font-base transition duration-200`}
                                onClick={() => { setMessage(`Xác nhận xoá ${Array.from(selectedRows.values()).reduce((total, set) => total + set.size, 0)} bưu cục - đại lý đã chọn?`); setOpenDelete(true) }}>
