...
const AccountTable = () => {
    ...
    const handleFetchData = async () => {
        setData(null)
        let response
        if (adminRole.includes(passData.account.role) || agencyRole.includes(passData.account.role)) {
            response = await accountOperation.search({})
    ...
    return (
        <>
            {openModal && <DetailAccount dataInitial={dataRow} onClose={() => setOpenModal(false)} reloadData={reloadData} />}
            {passData ? (
                data ? (
                    <CheckTable columnsData={columnsData} tableData={data} reloadData={reloadData} dataRow={dataRow} setDataRow={setDataRow} openModal={openModal} setOpenModal={setOpenModal} />
                ) : (
