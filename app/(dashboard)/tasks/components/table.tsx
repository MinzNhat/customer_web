...
const CheckTable = (props: Props) => {
    ...
    return (
        <Card className={"h-full w-full p-4 overflow-auto no-scrollbar"}>
            {/* {openAdd && <AddVehicle reloadData={reloadData} setOpenAdd={setOpenAdd} />} */}
            ...
            outline-none dark:bg-[#3A3B3C]  pl-1 pr-3`}
              ...
              linear justify-center rounded-lg font-medium dark:font-base transition duration-200`}
                            ...
                            <MdOutlineAddCircleOutline /><p className="flex gap-1">Thêm <p className="hidden sm:block">công việc</p></p>
                        </Button>
              ...
              linear justify-center rounded-lg font-medium dark:font-base transition duration-200`}
                                onClick={() => { setMessage(`Xác nhận xoá ${Array.from(selectedRows.values()).reduce((total, set) => total + set.size, 0)} công việc đã chọn?`); setOpenDelete(true) }}>
