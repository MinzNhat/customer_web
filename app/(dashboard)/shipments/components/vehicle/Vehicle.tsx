'use client'
import { OrdersOperation, VehicleOperation, Role } from "@/TDLib/main";
...
const VehicleTable = ({ selectedId, setOpen, reloadData2 }: { selectedId: any, setOpen: any, reloadData2: any }) => {
    ...
    const handleFetchData = async () => {
        ...
        if (adminRole.includes(passData.account.role)) {
            response = await vehicleOperation.searchByAdmin({})
            setData(response.data)
        } else if (agencyRole.includes(passData.account.role)) {
            response = await vehicleOperation.searchByAgency({})
            setData(response.data)
        } else {
    ...
    const handleSubmit = async () => {
        if (!selectedVehicle) {
            setMessage("Vui lòng chọn phương tiện")
            setOpenError(true)
            return;
        } else {
            setMessage(`Xác nhận thêm ${selectedId.length} lô hàng đã chọn vào phương tiện có mã: ${selectedVehicle}`)
            setOpenSubmit(true)
        }
    }

    const handleSubmit2 = async () => {
        const response = await vehicleOperation.addShipments(selectedVehicle, { shipmentIds: selectedId })
        setOpenSubmit(false)
        console.log(response)
        if (response.error) {
            ...
            setMessage("Thêm vào phương tiện thành công")
            setOpenReload(true)
        }
    }
    ...
    return (
        <DetailPopup
            ...
            {openReload && <NotiPopup message={message} onClose={() => { setOpenReload(false); reloadData2() }} />}
            ...
            {passData ? (
                data ? (
                    <CheckTable columnsData={columnsData} tableData={data} reloadData={reloadData} selectedId={selectedVehicle} setSelectedId={setSelectedVehicle} />
                ) : (
