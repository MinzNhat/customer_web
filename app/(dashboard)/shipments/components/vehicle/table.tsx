...
const CheckTable = (props: Props) => {
    ...
    const toggleRowSelection = (rowIndex: number) => {
        if (selectedRowIndex === rowIndex) {
            ...
            setSelectedId(tableData[rowIndex]?.vehicleId)
        }
    };
                                ...
                                <tr {...headerGroup.getHeaderGroupProps()} key={index}>
                                    {headerGroup.headers.map((column, index) => (
                                        <th
                                            ...
                                            key={index}
                                        >
                                            <div className={`text-xs font-bold tracking-wide text-gray-600 lg:text-xs whitespace-nowrap ${column.render("Header") === "detail" ? "text-end" : (column.render("Header") === "vehicle.busy" ? "text-center px-4" : "text-start pr-4 lg:pr-0")}`}>
